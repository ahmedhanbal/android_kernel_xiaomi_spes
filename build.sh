...
for VARIANT in "${VARIANTS[@]}"; do
  ...
  git checkout "$BRANCH"
	...
	# ---------------------------------
	if [ "$VARIANT" == "su" ]; then
 	     echo "Add KernelSU from backslashxx"
             curl -LSs "https://raw.githubusercontent.com/backslashxx/KernelSU/master/kernel/setup.sh" | bash -
	fi
  # --------------------------
  # Defconfig
  # --------------------------
  make $MAKE_ARGS vendor/bengal-perf_defconfig vendor/xiaomi/spes.config asano.config
  ...
  sed -i "s/^kernel.string=.*/kernel.string=angelo-asano-$VARIANT/" \
    "$ANYKERNEL_DIR/anykernel.sh"

  # --------------------------
  # Zip Packaging
  # --------------------------
  ZIPNAME="angelo-asano-${VARIANT}-${DATE_TIME}.zip"
...
echo "========================================="
ls -lh angelo-asano-*-${DATE_TIME}.zip
